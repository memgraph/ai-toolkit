...
To use [session enrichment](#session-enrichment) (entity extraction from session content):

```bash
pip install sessions-graph[enrichment]
```
...
enrichment](#session-enrichment) below for how they get linked.
...
Memories are mostly opaque text today. Session enrichment runs that content
...
[`CONTEXT.md`](./CONTEXT.md#language) for the **Session Enrichment** /
**Enrichable Content** / **Enrichment Status** terminology.

This requires the `sessions-graph[enrichment]` extra and an LLM API key
...
**Enrichment never runs inside the `SESSION_END` hook itself.** LightRAG
...
- On `SESSION_END`, `SessionsGraphConnector` cheaply marks the session
  `enrichment_status = 'pending'` — no LLM calls, safe inside the hook.
- The actual enrichment run happens out-of-band, via the CLI:

  ```bash
  # Enrich one session
  sessions-graph enrich --session s-abc123

  # Sweep every session still marked 'pending' (e.g. from cron)
  sessions-graph enrich --pending --limit 50
  ```
...
- Or, if you want it triggered automatically without a manual/cron step, opt
  ...
  session ends: pass `SessionsGraphConnector(graph, auto_enrich=True)`, or set
  `SESSIONS_GRAPH_AUTO_ENRICH=1` in the environment the connector is
  ...
  `pending` and `sessions-graph enrich --pending` is the reliable backfill.
...
summary = await graph.enrich_session("s-abc123", lightrag_wrapper=lightrag_wrapper)
...
`entity_workspace=` explicitly to `enrich_session()` to isolate them instead.
...
| Method | Description |
|---|---|
| `setup()` | Create constraints, text index, and enrichment indexes. Run once on first use. |
...
| `enrich_session(session_id, *, lightrag_wrapper, actions_graph=None, entity_workspace=None)` | Run session enrichment for one session. Returns an `EnrichmentSummary`. Requires the `enrichment` extra. |
| `get_pending_enrichment_sessions(*, limit=100)` | Return session IDs marked `enrichment_status = 'pending'`. |
