...
```bash
agent-context-graph doctor --runtime claude-code --connector skills-graph --connector actions-graph
```
...
printf '{"hook_event_name":"Stop","session_id":"doctor"}' \
  | agent-context-graph hook run claude-code --connector skills-graph --connector actions-graph --strict
```
